...
def dtw(dir_ret,query_name,utt_name,feat_query,feat_utt,smooth_param,fai,shift):
	...
	if(not os.path.exists(dir_ret+query_name+".ret")):
		file_query_name=open(dir_ret+query_name+".ret","w");
		file_query_name.writelines("detection results for "+query_name+"\n");
...
def main():
	...
	for query in feat_queries:
		for utt in feat_utts:
			queries_utts.append((query,utt));
	smooth_param=0.00001;		###parameter value for smoothing
	fai=1;
	shift=10;
	dir_ret=sys.argv[3];
	
	jobs = [job_server.submit(dtw,(dir_ret,query_utt[0][0],query_utt[1][0],query_utt[0][1],query_utt[1][1],smooth_param,fai,shift), (smooth_feat,compute_simi_matrix,dystep_ISA,getTime), ("numpy",)) for query_utt in queries_utts]

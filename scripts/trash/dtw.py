...
def dtw(query_name,utt_name,feat_query,feat_utt,smooth_param,fai):
	...
	print "---"+query_name+"/"+utt_name+": begin="+str(begin)+" end="+str(end)+" socre="+str(minDistance)+"\n";
...
def main():
	...
	feat_queries=[(os.path.basename(one),readFeat(one)) for one in files_featFile_query]; ###feature matrix of query
	feat_utts=[(os.path.basename(one),readFeat(one)) for one in files_featFile_utt]; ###feature matrix of utt
	smooth_param=0.00001;		###parameter value for smoothing
	fai=1;
	
	for query in feat_queries:
		for utt in feat_utts:
			out=dtw(query[0],utt[0],query[1],utt[1],smooth_param,fai);
